...
public class Trie
{
  ...
  public void insert(String key)
  {
    ...
    TrieNode prefix = getPrefixNode(key); // get prefix first
     int depth;
     for (depth = prefix.getDepth(); depth < key.length(); depth++){    
       prefix = prefix.createChild(key.charAt(depth)); // Fill in the missing nodes
      }
      prefix.setEndOfKey(true);
  ...
  private TrieNode getPrefixNode(String key)
  {
    ...
    int i = 0;
    while ( i < key.length()){ // iterate through all the keys in the trie
     if (current.getChild(key.charAt(i)) == null)
      return current;    // if no prefix is found, return the root 
     else
      current = current.getChild(key.charAt(i)); // current becomes first child
     i++;
    }
    return current;   //  key is already starting at root
  ...
  //  HELPER METHOD    DELETE THIS FOR POSTED CODE.
   ...
   // At each node visited, if the node is the end of a key, then that key is added to the list.
     
   private void traverse(TrieNode node, ArrayList<String> list){
    if ( node.isEndOfKey() ){
     list.add(node.toString());
    }
    for (int i = 0; i < TrieNode.NUMCHILDREN; i++){
     if (node.getChild((char) i) != null){
     traverse(node.getChild((char) i),list);
     }
    }
   }
  ...
  private class TrieNode
  {
   ...
   //helper  
   public int getDepth() // get the distance of the node from the root
   {
      return depth;
   }
